...
class TransmonAndreev(base.QubitBaseClass1d, serializers.Serializable, NoisySystem):
    r"""Class for the transmon qubit with Andreev Bound States. The Hamiltonian is
    ...
    """
    T = descriptors.WatchedProperty(float, "QUANTUMSYSTEM_UPDATE")
    ...
    def __init__(
        ...
        self._image_filename = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "qubit_img/fixed-transmon.jpg" #CHANGE THIS
        )

    @staticmethod
    def default_params() -> Dict[str, Any]:
        return {"T": 0.8, "Delta": 0.2, "EC": 0.3, "ng": 0.0, "ncut": 30, "truncated_dim": 10}
    ...
    def potential(self, delta: Union[float, ndarray]) -> ndarray:
        ...
        """
        return -self.Delta * np.sqrt(1-self.T* np.sin(delta/2)**2)
